...
void main() {
  ...
  testWidgets('opens notifications page from dashboard and clears all', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1170, 2652);
    tester.view.devicePixelRatio = 3.0;
    ...
    expect(
      find.byKey(const Key('dashboard_open_notifications')),
      findsOneWidget,
    );
    await tester.tap(find.byKey(const Key('dashboard_open_notifications')));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('notifications_title')), findsOneWidget);
    expect(find.text('Payment Successful'), findsOneWidget);
    expect(find.text('Wallet Topped Up'), findsOneWidget);
